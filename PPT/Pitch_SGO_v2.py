...
def main():
    ...
    #region SLIDE 3: O Conceito
    h += '<div class="sd"><div class="si">'
    h += '<h1 class="an" style="text-align:center;margin-bottom:1rem;">O que é o <span class="hl">SGO</span></h1>'
    h += '<p class="an" style="text-align:center;color:var(--mu);margin-bottom:2rem;font-size:1.2rem;">O SGO transforma conhecimento operacional em <strong>regras sistêmicas</strong>. Não é apenas um apontador de OS — é um <strong>mecanismo de decisão operacional</strong>.</p>'
    h += '<div class="an grid-3" style="margin-top:0;">'
    concl = [
        ('🗂️','Organiza a execução'),
        ('🚦','Prioriza atividades críticas'),
        ('🔒','Controla a aderência ao planejamento'),
        ('📸','Registra evidências'),
        ('🔄','Integra os resultados ao SAP'),
    ]
    for ic,t in concl:
        h += f'<div class="glass" style="padding:1.2rem 1.5rem;display:flex;align-items:center;gap:1rem;"><div style="font-size:1.8rem;">{ic}</div><h3 style="font-size:1.05rem;">{t}</h3></div>'
    h += '<div class="glass" style="padding:1.2rem 1.5rem;display:flex;align-items:center;gap:1rem;border-color:var(--gn);"><div style="font-size:1.8rem;">💎</div><h3 style="font-size:1.05rem;color:var(--gn)">Padroniza a boa prática</h3></div>'
    h += '</div></div></div>'
    #endregion
    ...
    fn = 'Pitch_Eletroeletronica_SGO_v2.html'
