...
def main():
    ...
    h += '<div class="tkc"><span class="tki">⚡ SGO ELETROELETRÔNICA MRS ⚡ GESTÃO OPERACIONAL ⚡ ROTEIRIZAÇÃO INTELIGENTE ⚡ GEOFENCING 2,0 KM ⚡ GPS OBRIGATÓRIO ⚡ PWA OFFLINE HTTPS ⚡ LOGIN PERSISTENTE ⚡ AUTOMAÇÃO SAP ⚡ AUDITORIA TME</span></div>'
    h += '<div class="tkc"><span class="tki">⚡ SGO ELETROELETRÔNICA MRS ⚡ GESTÃO OPERACIONAL ⚡ ROTEIRIZAÇÃO INTELIGENTE ⚡ GEOFENCING 2,0 KM ⚡ GPS OBRIGATÓRIO ⚡ PWA OFFLINE HTTPS ⚡ LOGIN PERSISTENTE ⚡ AUTOMAÇÃO SAP ⚡ AUDITORIA TME</span></div>'
    h += '</div></div>'
    ...
    for i in range(12): h += f'<div class="dot {"active" if i==0 else ""}"></div>'
    h += '</div><div class="sw">'
    #endregion
    ...
    h += '<div class="glass" style="border-left:4px solid var(--ac);padding:1.5rem;"><h3 class="hl">Inteligência Geográfica (Cálculo de Distância Real)</h3><p style="margin-top:0.5rem;color:var(--txt);">O motor cruza as coordenadas de GPS em tempo real do técnico com o nosso dicionário espacial da ferrovia. Com o botão <strong>"Filtrar"</strong>, entrega visualmente apenas as OSs dentro do raio de atuação escolhido (início em 1 km, ajustável).</p></div>'
    h += '</div>'
    ...
    h += '<li>Identifica qualquer OS <strong>Muito Alta</strong> (Segurança / Confiabilidade Crítica), independente da data.</li>'
    h += '<li><strong>Bloqueia</strong> as OSs de menor prioridade do mesmo grupo — que permanecem <strong>visíveis</strong> (sombreadas + 🔒), sem sumir da tela.</li>'
    h += '<li>Força a equipe a atacar e resolver a emergência antes de prosseguir com manutenções preventivas normais.</li></ul>'
    h += '</div></div></div></div>'
    #endregion

    #region SLIDE 6: Resiliência Extrema: Operação Offline (PWA HTTPS)
    h += '<div class="sd"><div class="si">'
    h += '<h1 class="an" style="text-align:center;margin-bottom:1rem;">Resiliência Extrema: <span class="hl-y">PWA Offline</span></h1>'
    h += '<p class="an" style="text-align:center;color:var(--mu);margin-bottom:3rem;font-size:1.2rem;">A operação continua fluída mesmo sem sinal de rádio ou 4G no trecho — via aplicativo instalado (PWA HTTPS), nunca por arquivo solto.</p>'
    h += '<div class="an grid-3">'
    h += '<div class="glass" style="text-align:center;"><div style="font-size:3rem;margin-bottom:1rem;">📡</div><h3>1. Publicar a Rota (PWA)</h3><p style="margin-top:1rem;color:var(--mu);">Na base, com internet, o gestor clica em <strong>"Publicar Rota PWA"</strong>. O técnico abre o link seguro (HTTPS) uma única vez e o pacote fica instalado no celular como um app.</p></div>'
    h += '<div class="glass" style="text-align:center;border-color:var(--yl);box-shadow:0 0 30px rgba(245,158,11,0.2);"><div style="font-size:3rem;margin-bottom:1rem;">📴</div><h3 class="hl-y">2. Modo Local Seguro</h3><p style="margin-top:1rem;color:var(--mu);">No trecho sem sinal, o PWA roda direto no aparelho em contexto seguro (HTTPS). O <strong>GPS do celular permanece ativo</strong> e os apontamentos e fotos ficam guardados localmente (IndexedDB).</p></div>'
    h += '<div class="glass" style="text-align:center;"><div style="font-size:3rem;margin-bottom:1rem;">🔄</div><h3>3. Sincronização</h3><p style="margin-top:1rem;color:var(--mu);">Ao reconectar, o app envia todos os apontamentos e fotos ao servidor de uma só vez. A gravação é idempotente: <strong>zero perda e zero duplicidade</strong>.</p></div>'
    h += '</div></div></div>'
    #endregion

    #region SLIDE 7: Governança e Antifraude (GPS obrigatório — EXIF removido)
    h += '<div class="sd"><div class="si">'
    ...
    h += '<div style="margin-bottom:1.8rem;display:flex;gap:1.5rem;align-items:flex-start;"><div style="font-size:2.5rem;">📍</div><div><h3 class="hl-g">Geofencing Estrito</h3><p style="color:var(--mu);margin-top:0.5rem;">A Baixa Eletrônica só é aceita se o GPS atestar que o aparelho está num raio máximo de <strong>2,0 km</strong> da coordenada oficial do ativo na linha (cálculo Haversine).</p></div></div>'
    h += '<div style="margin-bottom:1.8rem;display:flex;gap:1.5rem;align-items:flex-start;"><div style="font-size:2.5rem;">🛰️</div><div><h3 class="hl">GPS Obrigatório e Real</h3><p style="color:var(--mu);margin-top:0.5rem;">A fonte de localização é <strong>sempre o hardware</strong> do aparelho, em contexto HTTPS seguro. Coordenada <strong>(0,0) é rejeitada pela API (HTTP 400)</strong> — sem apontamento "fantasma", sem fallback frágil por foto.</p></div></div>'
    h += '<div style="display:flex;gap:1.5rem;align-items:flex-start;"><div style="font-size:2.5rem;">📸</div><div><h3 style="color:#A78BFA">Evidência Fotográfica</h3><p style="color:var(--mu);margin-top:0.5rem;">Cada baixa exige foto, tratada e enviada ao repositório na nuvem, formando o <strong>histórico visual auditável</strong> da malha.</p></div></div>'
    h += '</div>'
    ...
    #region SLIDE 10: Deploy 03/Jul — O que entrou no ar
    h += '<div class="sd"><div class="si">'
    h += '<h1 class="an" style="text-align:center;margin-bottom:1rem;">Deploy <span class="hl-g">03/Jul</span> — O que entrou no ar</h1>'
    h += '<p class="an" style="text-align:center;color:var(--mu);margin-bottom:2.5rem;font-size:1.2rem;">A rodada de estabilização que blindou a operação de campo.</p>'
    h += '<div class="an grid-3" style="gap:1.5rem;">'
    h += '<div class="glass" style="padding:1.5rem;border-left:4px solid var(--gn);"><div style="font-size:2.2rem;margin-bottom:0.8rem;">🛰️</div><h3 class="hl-g" style="font-size:1.1rem;">GPS Obrigatório</h3><p style="font-size:0.9rem;color:var(--mu);margin-top:0.6rem;">Fonte única: o hardware. Antifraude por foto (EXIF) aposentado. API rejeita (0,0) com HTTP 400.</p></div>'
    h += '<div class="glass" style="padding:1.5rem;border-left:4px solid var(--yl);"><div style="font-size:2.2rem;margin-bottom:0.8rem;">📲</div><h3 class="hl-y" style="font-size:1.1rem;">Distribuição PWA HTTPS</h3><p style="font-size:0.9rem;color:var(--mu);margin-top:0.6rem;">Publicar Rota → abrir 1x online → usar offline. Contexto seguro que libera o GPS mesmo sem sinal.</p></div>'
    h += '<div class="glass" style="padding:1.5rem;border-left:4px solid var(--ac);"><div style="font-size:2.2rem;margin-bottom:0.8rem;">🔐</div><h3 class="hl" style="font-size:1.1rem;">Login Persistente</h3><p style="font-size:0.9rem;color:var(--mu);margin-top:0.6rem;">Token seguro na sessão (12h). A câmera não derruba mais o acesso do técnico em campo.</p></div>'
    h += '<div class="glass" style="padding:1.5rem;border-left:4px solid var(--ac);"><div style="font-size:2.2rem;margin-bottom:0.8rem;">🎯</div><h3 class="hl" style="font-size:1.1rem;">Botão "Filtrar" + Raio 1 km</h3><p style="font-size:0.9rem;color:var(--mu);margin-top:0.6rem;">Fim do auto-refresh: busca por proximidade mais precisa, app mais leve e rápido.</p></div>'
    h += '<div class="glass" style="padding:1.5rem;border-left:4px solid var(--yl);"><div style="font-size:2.2rem;margin-bottom:0.8rem;">⏱️</div><h3 class="hl-y" style="font-size:1.1rem;">Baixa em Massa (Horário Único)</h3><p style="font-size:0.9rem;color:var(--mu);margin-top:0.6rem;">Um Data/Hora replicado a várias OS de uma vez. Filtro CI/SI também no offline.</p></div>'
    h += '<div class="glass" style="padding:1.5rem;border-left:4px solid var(--rd);"><div style="font-size:2.2rem;margin-bottom:0.8rem;">🔒</div><h3 class="hl-r" style="font-size:1.1rem;">OS Bloqueadas Visíveis</h3><p style="font-size:0.9rem;color:var(--mu);margin-top:0.6rem;">Muito Alta trava as menores (sombreado + 🔒), sem sumir. Baixas sem duplicidade na tela.</p></div>'
    h += '</div></div></div>'
    #endregion

    #region SLIDE 11: Impacto (Big Numbers)
    ...
    h += '<div class="glass"><h2 class="big-num ac">100%</h2><h3 style="margin-top:1rem;">Auditoria Integrada</h3><p style="font-size:0.9rem;color:var(--mu);">Governança garantida pelo sistema via GPS obrigatório e geofencing de 2,0 km.</p></div>'
    ...
    #region SLIDE 12: A Síntese (O Que, Como, Por Que)
    h += '<div class="sd"><div class="si" style="text-align:center;">'
    ...
    h += '<p style="color:var(--txt);line-height:1.6;font-size:1rem;">Levando tecnologia ao campo com: <strong>Roteirização por GPS</strong>, travas sistêmicas contra decisões erradas e <strong>PWA Offline (HTTPS)</strong> para garantir fluidez no trecho.</p>'
    h += '</div>'

    h += '<div class="glass" style="border-top: 4px solid var(--yl);">'
    h += '<div style="font-size:2.5rem;margin-bottom:1rem;">💎</div>'
    h += '<h3 class="hl-y" style="margin-bottom:1rem;">Padrão Ouro</h3>'
    h += '<p style="color:var(--txt);line-height:1.6;font-size:1rem;">Entregamos uma <strong>Governança Intocável</strong>: integração limpa com o SAP, auditoria por <strong>GPS obrigatório e geofencing de 2,0 km</strong> e medição gerencial do real Tempo de Execução (TME).</p>'
    h += '</div>'
    h += '</div>'
    ...
    h += '</div>'  # Fecha .sw
    h += '<script>'
    h += 'const TS=12; let cur=0; const sls=document.querySelectorAll(".sd"); const dts=document.querySelectorAll(".dot");'
