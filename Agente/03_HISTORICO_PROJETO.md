...
| 10.3.3 (online) | Upload de foto simultâneo (baixa em massa) derrubava a sessão em rede de campo fraca (`ClientDisconnect`/tela "Huh") | Tentativa de correção escondia o `st.file_uploader` das OS já prontas pra sequenciar o envio — **isso causou o Streamlit esquecer o arquivo já carregado** (widget só mantém valor de forma confiável se renderizado em todo rerun), gerando **perda real de OS** em produção (16/07/2026). Revertido, depois corrigido de novo usando `disabled=True` (widget sempre renderizado, nunca some) em vez de esconder — ver regra em `05_PADROES_TECNICOS.md` sobre testar mudanças de widget antes de produção |
