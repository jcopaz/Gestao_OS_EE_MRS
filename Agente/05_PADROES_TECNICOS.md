...
- **IndexedDB do pacote (v3, desde v20.0.0 / 05-09-2026):** `DB_NAME = "sgo_mrs_offline_prod"` versão **3**, store da fila `"fila"` com **`keyPath: "uid"`** (id sintético único por apontamento — `crypto.randomUUID()`), + índices `status_sync` e `os_id`.
  - **Por que mudou:** a v2 usava store `"apontamentos"` com `keyPath: "os_id"` — dois apontamentos da mesma OS (ou OS sem número → chave `""`) colidiam e o 2º `.put()` sobrescrevia o 1º sem erro ("N OS gravadas com sucesso" mas "1 na fila de envio", incidente 05/09).
  - **Migração:** `onupgradeneeded` v2→v3 **não é destrutivo** — cria o store `"fila"`, copia cada registro do `"apontamentos"` gerando um `uid` (`mig__<n>__<uuid>`), e **mantém o `"apontamentos"` intacto como backup congelado**. Não faz `deleteObjectStore` (o `deleteObjectStore` do código v2 apagava pendentes — classe do incidente 13/07).
  - `carregarOsGravadas()` lê os registros e monta o set por `os_id` (não por `getAllKeys()`, que agora devolve `uid`). O update pós-sync (`put({...item, status_sync: "sincronizado"})`) funciona porque o spread mantém o `uid`.
  - `gerar_html_offline` continua **filtrando "Ordem servico" em branco e deduplicando** antes de montar `OS_DATA` (card repetido confunde o técnico), e o JS de gravação recusa OS sem número. Não remover essas guardas.
  - Validado com `fake-indexeddb` (`scratchpad/teste_migracao_fila.mjs`): reproduz a colisão v2, prova migração não-destrutiva + uid único + status preservado + fim da colisão + update pós-sync. **Ainda vale a fumaça de 30 s num celular real** que tenha o pacote v2 com pendente: abrir o pacote novo 1x e conferir que a contagem "aguardando envio" não zerou.
