...
- 04/09 **reconciliação completa** (branch `reconciliacao-v18-para-main`, base = `f1d5fe8`/v18.2.0 + cherry-pick de `ebf2b94`, `b90abd8`, a lógica de `b0a5df6` sem a CSS (redundante — `f1d5fe8` já tem a melhor), a flag `EXIBIR_AGENDA_CALENDARIO` de `695b885`, e `9debaa7`) — vira **v19.0.0**. `py_compile` OK; `git diff f1d5fe8 HEAD -- app.py` confirmado como só as 5 mudanças genuinamente novas, nada de regressão. **Ainda pendente: validação visual em navegador/celular antes de promover pra `main`** (mesma exigência de qualquer mudança em widget/mapa, ver lições transversais).
