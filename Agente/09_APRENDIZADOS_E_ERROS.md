...
- Melhorias do mapa da v18.2.0 (raio verde, pino colorido por Segurança pendente, popup detalhado). ✅ *voltou junto na reconciliação (base = f1d5fe8 já tem tudo isso)*
- **245 linhas removidas de `render_tela_admin()` = a seção inteira "📥 Baixa Manual — NAPL"** (region 3.8.6, upload em lote de baixa NAPL com as validações de truncamento de `causa_nrav`/`texto_confirmacao` do incidente de 26/08) — sumiu por completo, inclusive a opção de permissão `"Baixa Manual NAPL"` em `opcoes_gov`. ✅ *voltou junto na reconciliação*
- **Bônus descoberto só na reconciliação completa (não estava na lista original):**
  - `COORDENADAS_FIXAS` (em `app.py`) voltou a não ter `IPN`/`IQC` e com a coordenada antiga/errada de `ICQ` (`-23.926493,-46.402720` em vez de `-23.91531...,-46.41890...`) — exatamente o que o incidente de 26/08/2026 tinha corrigido. **`api.py` (deploy separado) não foi tocado pelo upload e já estava com os valores certos** — ou seja, o geofence de campo (que usa `api.py`) nunca ficou vulnerável; só o mapa/resolução de pátio dentro do próprio `app.py` regrediu. ✅ *corrigido na reconciliação (herda de f1d5fe8)*
  - `max_chars` do campo de observação no fluxo NRAV manual voltou de 40 pra 38 (o fix de 26/08, `StringDataRightTruncation`, tinha corrigido pra 40). ✅ *corrigido na reconciliação*
...
- 04/09 **reconciliação completa** (branch `reconciliacao-v18-para-main`, base = `f1d5fe8`/v18.2.0 + cherry-pick de `ebf2b94`, `b90abd8`, a lógica de `b0a5df6` sem a CSS (redundante — `f1d5fe8` já tem a melhor), a flag `EXIBIR_AGENDA_CALENDARIO` de `695b885`, e `9debaa7`) — vira **v19.0.0**. `py_compile` OK; `git diff f1d5fe8 HEAD -- app.py` confirmado como só as 5 mudanças genuinamente novas, nada de regressão. **Mergeada em `main` no mesmo dia (04/09), a pedido do Julio, sem validação visual em navegador** (confiança baseada em `py_compile` + diff textual verificado hunk a hunk) — checklist de conferência visual pós-deploy recomendado mesmo assim (ver lição 13/07 acima sobre o mesmo padrão).
...
5. **`git merge` de uma branch de reconciliação pode reintroduzir a própria regressão em silêncio, sem conflito nenhum.** No merge `reconciliacao-v18-para-main` → `main`, o merge-base (3-way) era `f1d5fe8` — para qualquer hunk que **só o rollback tinha mudado** (a reconciliação nunca tocou ali, porque não fazia parte dos 5 fixes novos), o git viu "só um lado mudou" e aplicou a mudança do `main`... que era a própria regressão do rollback. Zero conflito reportado, mas `app.py` pós-merge ficou com 412 linhas diferentes da branch já auditada. **Detectado só porque o hábito virou checar `git diff <branch-boa> <destino> -- app.py` (deveria dar vazio) logo depois do merge, antes de commitar** — resolvido com `git checkout <branch-boa> -- app.py` (substitui o arquivo inteiro pela versão já auditada, em vez de confiar na resolução automática). **Regra prática:** ao mergear uma branch que existe especificamente para desfazer um rollback/regressão em massa, nunca confiar em "o merge terminou sem conflito" como sinal de que deu certo — sempre `git diff` pós-merge contra a branch de origem pra confirmar que o resultado é byte-idêntico onde deveria ser.
...
**Correção estrutural (v20.0.0, 05/09):** store da fila passou a ser `"fila"` com `keyPath: "uid"` (id sintético único por apontamento) + índices `status_sync` e `os_id`. Migração v2→v3 **não destrutiva**: `onupgradeneeded` cria o store novo, copia cada registro do `"apontamentos"` gerando um `uid`, e **mantém o `"apontamentos"` como backup congelado** (nada de `deleteObjectStore`). `carregarOsGravadas()` passou a montar o set por `os_id` (não por `getAllKeys()`, que agora devolve `uid`). Validado com `fake-indexeddb` (17 asserts: reproduz a colisão v2, prova migração + uid único + status preservado + fim da colisão + update pós-sync). Falta só a fumaça de 30 s num celular real com pacote v2 pendente.
...
**Correção (v20.1.2 → v20.1.3):** `init_connection_pool()` mantido igual (a retentativa interna está certa). A chamada no topo do script virou `_abrir_pool_ou_tela_de_espera()`:
- **v20.1.2** — se o pool não abrir, `st.error` limpo + botão **"🔄 Tentar novamente"** (`init_connection_pool.clear()` + `st.rerun()`) + `st.stop()`, em vez do traceback do psycopg2 na tela do técnico.
- **v20.1.3** — `_diagnostico_falha_neon(str(e))` classifica o texto do erro em 3 casos (credencial inválida / **limite do plano Free** / sem-resposta genérico → "provável limite Free") e a tela mostra a mensagem certa + o aviso de que o **campo não para** (Rota PWA offline segue funcionando). `_alertar_dev_falha_banco()` faz um POST best-effort num webhook (`st.secrets["ALERTA_WEBHOOK_URL"]` — Teams/Slack/Discord/Power Automate), no máximo 1×/30 min (global `_ALERTA_DEV_ULTIMO_ENVIO`), com `try/except` que nunca levanta; a tela diz "desenvolvedor notificado" só se o webhook respondeu 2xx, senão "avise a coordenação".

**Isso não conserta a disponibilidade do Neon — só a UX da falha e o aviso ao dev.**
