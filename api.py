...
if not allowed_origins:
    raise RuntimeError("Variável de ambiente ALLOWED_ORIGINS não configurada ou vazia.")
...
def get_decimal_from_dms(dms, ref):
    """
    Converte coordenadas EXIF para decimal com tolerância a frações/tipos EXIF.
    """
    try:
        def _to_float(valor):
            # Alguns EXIFs vêm como tupla (num, den), outros como objeto racional
            if isinstance(valor, tuple) and len(valor) == 2 and valor[1] != 0:
                return float(valor[0]) / float(valor[1])
            return float(valor)

        graus = _to_float(dms[0])
        minutos = _to_float(dms[1])
        segundos = _to_float(dms[2])
...
def extrair_gps_exif(imagem_pil: Image.Image):
    """
    Extrai latitude/longitude do EXIF da imagem, se existirem.
    """
    try:
...
def upload_foto_supabase(arquivo_bytes: bytes, nome_arquivo: str) -> str:
    """
    Faz upload da imagem ao Supabase Storage já comprimida/orientada.
    Retorna a URL pública se sucesso, ou string vazia se falhar.
    """
    ...
    try:
        ...
        cur.execute(
            ...
            (
                str(os_id),
                str(status),
                str(realizado_em_str),
                str(coordenacao),
                str(concluido_por),
                str(geolocalizacao_baixa),
                str(equipe),
                str(data_inicio),
                str(hora_inicio),
                str(data_fim),
...
async def sincronizar_baixa_offline(
    ...
    if dist_km > 2.0:
        raise HTTPException(
            status_code=403,
            detail=(
                f"Bloqueio Geográfico: O apontamento foi realizado a "
                f"{dist_km:.1f}km do ativo (Limite máximo: 2.0km). Verifique seu GPS."
            )
        )

    # 4) Datas / horários
    hora_apontamento = datetime.fromisoformat(
        data_hora_local.replace("Z", "+00:00")
    ...
    if url_supabase:
        upsert_evidencia(
            ativo=ativo_id,
            atividade="Baixa Offline",
            foto_url=url_supabase,
            os_referencia=os_id,
            concluido_por=usuario,
            geolocalizacao=geo_string
        )
        foto_b64 = ""
    else:
