...
def formatar_dt_br(dt: datetime) -> str: return dt.strftime("%d/%m/%Y %H:%M")
...
async def sincronizar_baixa_offline(os_id: str = Form(...), ativo_id: str = Form(...), usuario: str = Form(...), lat_browser: float = Form(...), lon_browser: float = Form(...), data_hora_local: str = Form(...)):
    ...
    dist_km = haversine_vectorized(lat_browser, lon_browser, pd.Series([lat_ativo]), pd.Series([lon_ativo])).iloc[0]
    ...
    if delta_minutos < 10 and dist_km > 5.0:
        raise HTTPException(status_code=403, detail="Fraude detectada: Deslocamento impossível no tempo informado.")
    
    upsert_baixa(
        os_id=os_id, status="Realizado", realizado_em_str=formatar_dt_br(hora_apontamento), 
        coordenacao="Sincronização Offline", concluido_por=usuario, 
        geolocalizacao_baixa=f"Offline Sync (Lat: {lat_browser:.4f}, Lon: {lon_browser:.4f})",
        equipe="Sozinho", data_inicio=hora_apontamento.strftime("%d/%m/%Y"), 
        hora_inicio=hora_apontamento.strftime("%H:%M:%S"), data_fim=hora_apontamento.strftime("%d/%m/%Y"), 
        hora_fim=hora_apontamento.strftime("%H:%M:%S")
    )
    return {"status": "sucesso", "os_id": os_id, "dist_km": round(dist_km, 2), "auditoria": "OK"}