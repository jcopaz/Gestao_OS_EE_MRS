...
#region 10.3.5: Cronograma de Execução de Campo (Tabela/PDF)
            ...
            if not df_recomendado.empty:
                ...
                with col_btn_crono:
                    ...
                    try:
                        ...
                        def gerar_pdf_cronograma(df_pdf, colunas):
                            ...
                            # 🔥 CORREÇÃO DO PDF: Substitui "Enters" (\n) do SAP por tags HTML que o PDF entende
                            for _, row in df_pdf[colunas].iterrows(): 
                                data_rows.append([Paragraph(str(v).replace('\n', '<br/>').replace('\r', ''), style_cell) for v in row.values])
                ...
                st.dataframe(
                    ...
                    use_container_width=False, # 🔥 ATIVA A BARRA HORIZONTAL
                    width=2000, 
                    height=350, 
                    hide_index=True, 
                    column_order=colunas_exibir,
                    column_config={
                        "OS": st.column_config.TextColumn("OS", width=90),
                        "Data da Programação": st.column_config.TextColumn("Data Prog.", width=100),
                        "Patio": st.column_config.TextColumn("Pátio", width=80),
                        "Ativo": st.column_config.TextColumn("Ativo", width=130),
                        "Criticidade": st.column_config.TextColumn("Criticidade", width=100),
                        "Classificação": st.column_config.TextColumn("Classificação", width=180),
                        "Descrição Longa": st.column_config.TextColumn("Descrição Longa", width=900) # 🔥 ESPAÇO GIGANTE PARA O TEXTO
                    }
                )
            else: 
                st.info("Nenhuma OS pendente localizada dentro do raio de atuação selecionado.")
